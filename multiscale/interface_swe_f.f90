subroutine states(  idir, qx, qy, ng, dx, dt, &
  ...
  integer, intent(in) :: ih, iu, ix, nvar, nspec
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        if (idir == 1) then
           eval(0:ns-1) = [q(iu) - cs, q(iu) + cs]

           lvec(0,0:ns-1) = [ cs, -q(ih)]
           lvec(1,0:ns-1) = [ cs, q(ih)]

           rvec(0,0:ns-1) = [ q(ih), -cs]
           rvec(1,0:ns-1) = [ q(ih), cs]
...
subroutine riemann_Roe(idir, qx, qy, ng, &
  ...
  integer, intent(in) :: idir
  integer, intent(in) :: qx, qy, ng
  integer, intent(in) :: nvar, ih, ixmom, ihX, nspec
  ...
  double precision :: h_l, un_l, c_l
  double precision :: h_r, un_r, c_r
  ...
  do j = jlo-1, jhi+1
     do i = ilo-1, ihi+1
        ...
        lambda_roe(0:1) = [un_roe - c_roe, un_roe + c_roe]
        if (idir == 1) then
           alpha_roe(0:1) = [0.5d0*(delta(ih) - U_roe(ih)/c_roe*delta(ixmom)), &
                0.0d0]

           K_roe(0, 0:1) = [1.0d0, un_roe - c_roe]
           K_roe(1, 0:1) = [1.0d0, un_roe + c_roe]
           ...
           call consFlux(idir, g, ih, ixmom, ihX, nvar, nspec, &
                U_l(i,j,:), F(i,j,:))
           ...
           if (abs(lambda_roe(1)) < tol) then
              lambda_roe(1) = lambda_roe(1) * (u_star + c_star - lambda_roe(1)) / &
...
subroutine riemann_HLLC(idir, qx, qy, ng, &
  ...
  double precision :: h_l, un_l
  double precision :: h_r, un_r
  double precision :: xn(nspec)
...
subroutine consFlux(idir, g, ih, ixmom, ihX, nvar, nspec, U_state, F)

  implicit none

  integer, intent(in) :: idir
  double precision, intent(in) :: g
  integer, intent(in) :: ih, ixmom, ihX, nvar, nspec
