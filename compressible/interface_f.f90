subroutine states(idir, qx, qy, ng, dx, dt, &
                  ...
                  gamma, &
                  r, u, v, p, phi, &
                  ldelta_r, ldelta_u, ldelta_v, ldelta_p, ldelta_phi, &
                  q_l, q_r)
  ...
  double precision, intent(inout) :: phi(0:qx-1, 0:qy-1)
...
!f2py intent(out) :: q_l, q_r
  ...
  !
  ! Taking our state vector as Q = (rho, u, v, p, phi)^T, the eigenvalues
  ! are u - c, u, u + c.
  !
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        q(:) = [r(i,j), u(i,j), v(i,j), p(i,j), phi(i,j)]

        cs = sqrt(gamma*p(i,j)/r(i,j))

        ! compute the eigenvalues and eigenvectors
        if (idir == 1) then
           eval(:) = [u(i,j) - cs, u(i,j), u(i,j), u(i,j) + cs, u(i,j)]
           ...
           lvec(3,:) = [ 0.0d0, 0.5d0*r(i,j)/cs,  0.0d0, 0.5d0/(cs*cs), 0.0d0 ]
           lvec(4,:) = [ 0.0d0, 0.0d0,            0.0d0, 0.0d0,         1.0d0 ]

           rvec(0,:) = [1.0d0, -cs/r(i,j), 0.0d0, cs*cs, 0.0d0 ]
           rvec(1,:) = [1.0d0, 0.0d0,      0.0d0, 0.0d0, 0.0d0 ]
           rvec(2,:) = [0.0d0, 0.0d0,      1.0d0, 0.0d0, 0.0d0 ]
           rvec(3,:) = [1.0d0, cs/r(i,j),  0.0d0, cs*cs, 0.0d0 ]
           rvec(4,:) = [0.0d0, 0.0d0,      0.0d0, 0.0d0, 1.0d0 ]

        else
           eval = [v(i,j) - cs, v(i,j), v(i,j), v(i,j) + cs, v(i,j)]

           lvec(0,:) = [ 0.0d0, 0.0d0, -0.5d0*r(i,j)/cs, 0.5d0/(cs*cs), 0.0d0 ]
           lvec(1,:) = [ 1.0d0, 0.0d0, 0.0d0,            -1.0d0/(cs*cs),0.0d0 ]
           lvec(2,:) = [ 0.0d0, 1.0d0, 0.0d0,            0.0d0,         0.0d0 ]
           lvec(3,:) = [ 0.0d0, 0.0d0, 0.5d0*r(i,j)/cs,  0.5d0/(cs*cs), 0.0d0 ]
           lvec(4,:) = [ 0.0d0, 0.0d0, 0.0d0,            0.0d0,         1.0d0 ]

           rvec(0,:) = [1.0d0, 0.0d0, -cs/r(i,j), cs*cs, 0.0d0 ]
           rvec(1,:) = [1.0d0, 0.0d0, 0.0d0,      0.0d0, 0.0d0 ]
           rvec(2,:) = [0.0d0, 1.0d0, 0.0d0,      0.0d0, 0.0d0 ]
           rvec(3,:) = [1.0d0, 0.0d0, cs/r(i,j),  cs*cs, 0.0d0 ]
           rvec(4,:) = [0.0d0, 0.0d0, 0.0d0,      0.0d0, 1.0d0 ]
...
subroutine riemann_cgf(idir, qx, qy, ng, &
  ...
  integer, intent(in) :: idir
  integer, intent(in) :: qx, qy, ng
  integer, intent(in) :: nvar, idens, ixmom, iymom, iener, iphi
  double precision, intent(in) :: gamma
...
!f2py intent(out) :: F
  ...
  double precision, parameter :: smallphi = 1.e-10

  double precision :: rho_l, un_l, ut_l, rhoe_l, p_l, phi_l
  double precision :: rho_r, un_r, ut_r, rhoe_r, p_r, phi_r

  double precision :: rhostar_l, rhostar_r, rhoestar_l, rhoestar_r
  double precision :: phistar
  ...
  do j = jlo-1, jhi+1
     do i = ilo-1, ihi+1
        ...
        phi_l  = U_l(i,j,iphi)
        phi_l  = max(phi_l, smallphi)

        phi_r  = U_r(i,j,iphi)
        phi_r  = max(phi_r, smallphi)
        ...
        if (ustar > 0.0d0) then
           ...
           if (pstar > p_l) then
              ...
              if (lambda_l < 0.0d0 .and. lambdastar_l < 0.0d0) then
                 ...
                 phi_state  = alpha*phistar    + (1.0d0 - alpha)*phi_l
              endif
           ...
           if (pstar > p_r) then
              ...
              if (lambda_r < 0.0d0 .and. lambdastar_r < 0.0d0) then
                 ...
                 phi_state  = alpha*phistar    + (1.0d0 - alpha)*phi_r
...
!f2py intent(out) :: F
  ...
  double precision :: ustar, pstar, phistar
  ...
  do j = jlo-1, jhi+1
     do i = ilo-1, ihi+1
        ...
        if (S_r <= 0.0d0) then
           ...
           U_state(iphi) = phi_r*HLLCfactor/rho_r
           ...
           U_state(idens) = HLLCfactor
           U_state(iphi) = phi_l*HLLCfactor/rho_l
