    ...
    for (int i = ilo-1; i < ihi+1; i++) {
        for (int j = jlo-1; j < jhi+1; j++) {
            ...
            if (S_r <= 0.0) {
               ...
               U_state[iener] = HLLCfactor*(U_r[idx+iener]/rho_r +
               ...
               U_state[iener] = HLLCfactor*(U_l[idx+iener]/rho_l +
    ...
    for (int i = ilo-1; i < ihi+1; i++) {
        for (int j = jlo-1; j < jhi+1; j++) {
            ...
            divU_x = (u[idx] - u[(i-1)*qy+j])/dx + \
                 0.25*(v[idx+1] + v[(i-1)*qy+j+1] - v[idx-1] - v[(i-1)*qy-1])/dy;

            avisco_x[idx] = cvisc*fmax(-divU_x*dx, 0.0);

            // now the y-interface value
            divU_y = 0.25*(u[(i+1)*qy+j] + u[(i+1)*qy+j-1] - u[(i-1)*qy+j] - u[(i-1)*qy+j-1])/dx + \
                 (v[idx] - v[idx-1])/dy;
