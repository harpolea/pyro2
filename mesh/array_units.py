...
from mesh.array_indexer import _buf_split
...
class UnitsArrayIndexer(unyt.unyt_array):
    ...
    def ip_jp(self, ishift, jshift, buf=0, n=0, s=1):
        ...
        if c == 2:
            return unyt.unyt_array(self[self.g.ilo-bxlo+ishift:self.g.ihi+1+bxhi+ishift:s,
    ...
    def lap(self, n=0, buf=0):
        ...
        l = (self.ip(-1, n=n, buf=buf) - 2*self.vi(n=n, buf=buf) + self.ip(1, n=n, buf=buf))/self.g.dx**2 + \
            (self.jp(-1, n=n, buf=buf) - 2*self.vi(n=n, buf=buf) + self.jp(1, n=n, buf=buf))/self.g.dy**2
        return l
    ...
    def copy(self):
        """
        make a copy of the array, defined on the same grid and with
        the same units
        """
