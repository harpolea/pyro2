...
def init_data(my_data, rp):
    """ initialize the swirly SR bubble problem """

    msg.bold("initializing the swirly problem...")
    ...
    if not isinstance(my_data, patch.CellCenterData2d):
        print("ERROR: patch invalid in swirly.py")
    ...
    dens_base = rp.get_param("swirly.dens_base")
    dens_left = rp.get_param("swirly.dens_left")
    eint_left = rp.get_param("swirly.eint_left")

    x_pert = rp.get_param("swirly.x_pert")
    y_pert = rp.get_param("swirly.y_pert")
    r_pert = rp.get_param("swirly.r_pert")
    pert_amplitude_factor = rp.get_param("swirly.pert_amplitude_factor")
    u_vel = rp.get_param("swirly.u_vel")
    omega = rp.get_param("swirly.omega")
    ...
    while i <= myg.ihi:
        ...
        while j <= myg.jhi:
            ...
            if (r <= r_pert):
                ...
                X[i,j] = 0.0
                u[i,j] = -omega * (myg.y[j] - y_pert)
                v[i,j] = omega * (myg.x[i] - x_pert)
