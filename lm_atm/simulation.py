...
class Simulation(NullSimulation):
    ...
    def initialize(self):
        ...
        alpha = self.base_data.grid.scratch_array()
        ...
        exec(self.problem_name + '.init_data(self.cc_data, self.base_data, \
             self.rp, self.metric)')
    ...
    def calcConstraint(self, zeta):
        ...
        constraint.d[:,:] = zeta.v2d(buf=zeta.ng) * (S.d - dp0dt[np.newaxis, :])
    ...
    def evolve(self):
        ...
        ldelta_rx.d[:,:] = limitFunc(1, D.d, myg)
        ldelta_ex = myg.scratch_array()
        ldelta_ex.d[:,:] = limitFunc(1, Dh.d, myg)
        ldelta_r0x = myg.scratch_array()
        ldelta_r0x.d[:,:] = limitFunc(1, D02d.d, myg)
        ldelta_e0x = myg.scratch_array()
        ldelta_e0x.d[:,:] = limitFunc(1, Dh02d.d, myg)
        ldelta_ux = myg.scratch_array()
        ldelta_ux.d[:,:] = limitFunc(1, u.d, myg)
        ldelta_vx = myg.scratch_array()
        ldelta_vx.d[:,:] = limitFunc(1, v.d, myg)

        ldelta_ry = myg.scratch_array()
        ldelta_ry.d[:,:] = limitFunc(2, D.d, myg)
        ldelta_ey = myg.scratch_array()
        ldelta_ey.d[:,:] = limitFunc(2, Dh.d, myg)
        ldelta_r0y = myg.scratch_array()
        ldelta_r0y.d[:,:] = limitFunc(2, D02d.d, myg)
        ldelta_e0y = myg.scratch_array()
        ldelta_e0y.d[:,:] = limitFunc(2, Dh02d.d, myg)
        ldelta_uy = myg.scratch_array()
        ldelta_uy.d[:,:] = limitFunc(2, u.d, myg)
        ldelta_vy = myg.scratch_array()
        ldelta_vy.d[:,:] = limitFunc(2, v.d, myg)
        ...
        ldelta_xiy = myg.scratch_array()
        ldelta_xix.d[:,:] = limitFunc(1, xi2d.d, myg)
        ldelta_xiy.d[:,:] = limitFunc(2, xi2d.d, myg)
        ...
        if np.max(M) > 0.2:
            msg.bold('\nWarning!')
            print('Mach number is ', np.max(M),
                  '- simulation will be less accurate.\n')
        ...
        constraint = self.calcConstraint(zeta)
        mg.init_RHS((div_zeta_U.v(buf=1) - constraint.v(buf=1)) /self.dt)
