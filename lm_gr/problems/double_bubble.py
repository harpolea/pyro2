...
def init_data(my_data, aux_data, base, rp, metric):
    ...
    x1_pert = rp.get_param("bubble.x1_pert")
    y1_pert = rp.get_param("bubble.y1_pert")
    r1_pert = rp.get_param("bubble.r1_pert")
    x2_pert = rp.get_param("bubble.x2_pert")
    y2_pert = rp.get_param("bubble.y2_pert")
    r2_pert = rp.get_param("bubble.r2_pert")
    ...
    r1 = np.sqrt((myg.x2d - x1_pert)**2  + (myg.y2d - y1_pert)**2)
    r2 = np.sqrt((myg.x2d - x2_pert)**2  + (myg.y2d - y2_pert)**2)

    idx = (r1 <= r1_pert) + (r2 <= r2_pert)
