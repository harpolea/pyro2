...
subroutine mac_vels(qx, qy, ng, dx, dy, dt, r, &
                    u, v, &
                    ldelta_ux, ldelta_vx, &
                    ldelta_uy, ldelta_vy, &
                    gradp_x, gradp_y, &
...
!f2py intent(in) :: r, u, v, gradp_x, gradp_y
...
!f2py intent(out) :: u_MAC, v_MAC
  ...
  call get_interface_states(qx, qy, ng, dx, dy, dt, r, &
                            u, v, &
                            ldelta_ux, ldelta_vx, &
                            ldelta_uy, ldelta_vy, &
                            gradp_x, gradp_y, &
...
subroutine states(qx, qy, ng, dx, dy, dt, r, &
                  u, v, &
                  ldelta_ux, ldelta_vx, &
                  ldelta_uy, ldelta_vy, &
                  gradp_x, gradp_y, &
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
subroutine rho_states(qx, qy, ng, dx, dy, dt, x, r, &
                      rho, u_MAC, v_MAC, &
                      ldelta_rx, ldelta_ry, &
                      rho_xint, rho_yint)
...
!f2py depend(qx, qy) :: x, r, rho, u_MAC, v_MAC
!f2py depend(qx, qy) :: ldelta_rx, ldelta_ry
!f2py intent(in) :: x, r, rho, u_MAC, v_MAC
...
!f2py intent(out) :: rho_xint, rho_yint
  ...
  double precision :: ubar, vbar, u_x, v_y, rhov_y, rhou_x
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        ! u on x-edges
        rho_xl(i+1,j) = rho(i,j) + 0.5d0*(1.0d0 - dtdx*ubar)*ldelta_rx(i,j)/ (r(i,j) + 0.5d0*dx)
        rho_xr(i  ,j) = rho(i,j) - 0.5d0*(1.0d0 + dtdx*ubar)*ldelta_rx(i,j)/ (r(i,j) + 0.5d0*dx)
        !rho_xl(i+1,j) = rho(i,j) + 0.5d0*(1.0d0 - dtdx*u_MAC(i+1,j))*ldelta_rx(i,j) / (r(i,j) + 0.5d0*dx)
        !rho_xr(i  ,j) = rho(i,j) - 0.5d0*(1.0d0 + dtdx*u_MAC(i,j))*ldelta_rx(i,j) / (r(i,j) - 0.5d0*dx)
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        u_x = (u_MAC(i+1,j) * sin(x(i,j) + 0.5d0*dx) - u_MAC(i,j)*sin(x(i,j) - 0.5d0*dx))/(dx * r(i,j) * sin(x(i,j)))
        v_y = (v_MAC(i,j+1)*(r(i,j)+0.5d0*dy)**2 - v_MAC(i,j)*(r(i,j)-0.5d0*dy)**2)/(dy * r(i,j)**2)
        ...
        rhov_y = (rho_yint(i,j+1) * v_MAC(i,j+1) * &
                 (r(i,j) + 0.5d0 * dy)**2 - rho_yint(i,j) * &
                 v_MAC(i,j) * (r(i,j) - 0.5d0 * dy)**2) / (dy * r(i,j)**2)

        rho_xl(i+1,j) = rho_xl(i+1,j) - 0.5d0*dt*(rhov_y + rho(i,j)*u_x)
        rho_xr(i  ,j) = rho_xr(i  ,j) - 0.5d0*dt*(rhov_y + rho(i,j)*u_x)

        ! (rho u)_x is the transverse term for the y-interfaces
        ! rho v_y is the non-advective piece for the y-interfaces
        rhou_x = (rho_xint(i+1,j) * u_MAC(i+1,j) * sin(x(i,j) + &
                  0.5d0  * dx) - rho_xint(i,j) * u_MAC(i,j) * &
                  sin(x(i,j) - 0.5d0 * dx) )/(dx * r(i,j) * sin(x(i,j)) )

        rho_yl(i,j+1) = rho_yl(i,j+1) - 0.5d0*dt*(rhou_x + rho(i,j)*v_y)
        rho_yr(i,j  ) = rho_yr(i,j  ) - 0.5d0*dt*(rhou_x + rho(i,j)*v_y)
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
subroutine psi_states(qx, qy, ng, dx, dy, dt, r, &
                      psi, u_MAC, v_MAC, &
                      ldelta_px, ldelta_py, source,&
                      psi_xint, psi_yint)
  ...
  ! 0-based indexing to match python
  double precision, intent(inout) :: r(0:qx-1, 0:qy-1)
  double precision, intent(inout) :: psi(0:qx-1, 0:qy-1)
  double precision, intent(inout) :: u_MAC(0:qx-1, 0:qy-1)
  double precision, intent(inout) :: v_MAC(0:qx-1, 0:qy-1)

  double precision, intent(inout) :: ldelta_px(0:qx-1, 0:qy-1)
  double precision, intent(inout) :: ldelta_py(0:qx-1, 0:qy-1)
  double precision, intent(inout) :: source(0:qx-1, 0:qy-1)

  double precision, intent(out) :: psi_xint(0:qx-1, 0:qy-1), psi_yint(0:qx-1, 0:qy-1)

!f2py depend(qx, qy) :: r, psi, u_MAC, v_MAC
!f2py depend(qx, qy) :: ldelta_px, ldelta_py, source
!f2py intent(in) :: r, psi, u_MAC, v_MAC
!f2py intent(in) :: ldelta_px, ldelta_py, source
!f2py intent(out) :: psi_xint, psi_yint

  double precision :: psi_xl(0:qx-1, 0:qy-1), psi_xr(0:qx-1, 0:qy-1)
  double precision :: psi_yl(0:qx-1, 0:qy-1), psi_yr(0:qx-1, 0:qy-1)
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        ! u on x-edges
        psi_xl(i+1,j) = psi(i,j) + 0.5d0*(1.0d0 - dtdx*ubar)*ldelta_px(i,j) / (r(i,j) + 0.5*dx)
        psi_xr(i  ,j) = psi(i,j) - 0.5d0*(1.0d0 + dtdx*ubar)*ldelta_px(i,j) / (r(i,j) + 0.5*dx)
        !psi_xl(i+1,j) = psi(i,j) + 0.5d0*(1.0d0 - dtdx*u_MAC(i+1,j))*ldelta_px(i,j) / (r(i,j) + 0.5*dx)
        !psi_xr(i  ,j) = psi(i,j) - 0.5d0*(1.0d0 + dtdx*u_MAC(i,j))*ldelta_px(i,j) / (r(i,j) - 0.5*dx)
  ...
  ! we upwind based on the MAC velocities
  call upwind(qx, qy, ng, psi_xl, psi_xr, u_MAC, psi_xint)
  call upwind(qx, qy, ng, psi_yl, psi_yr, v_MAC, psi_yint)
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        psi_xl(i+1,j) = psi_xl(i+1,j) - 0.5d0 * dt * vbar * psi_y + 0.5d0 * dt * source(i,j)
        psi_xr(i  ,j) = psi_xr(i  ,j) - 0.5d0 * dt * vbar * psi_y + 0.5d0 * dt * source(i,j)

        ! u psi_x is the transverse term for the y-interfaces
        psi_x = (psi_xint(i+1,j) - psi_xint(i,j))/(dx * r(i,j))

        psi_yl(i,j+1) = psi_yl(i,j+1) - 0.5d0 * dt * ubar * psi_x + 0.5d0 * dt * source(i,j)
        psi_yr(i,j  ) = psi_yr(i,j  ) - 0.5d0 * dt * ubar * psi_x + 0.5d0 * dt * source(i,j)
...
end subroutine psi_states


!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
subroutine get_interface_states(qx, qy, ng, dx, dy, dt, r, &
                                u, v, &
                                ldelta_ux, ldelta_vx, &
                                ldelta_uy, ldelta_vy, &
                                gradp_x, gradp_y, &
  ...
  do j = jlo-2, jhi+2
     do i = ilo-2, ihi+2
        ...
        ! u on x-edges
        u_xl(i+1,j) = u(i,j) + 0.5d0*(1.0d0 - dtdx*u(i,j))*ldelta_ux(i,j) / (r(i,j) + 0.5*r(i,j))
        u_xr(i  ,j) = u(i,j) - 0.5d0*(1.0d0 + dtdx*u(i,j))*ldelta_ux(i,j)/ (r(i,j) - 0.5*r(i,j))

        ! v on x-edges
        v_xl(i+1,j) = v(i,j) + 0.5d0*(1.0d0 - dtdx*u(i,j))*ldelta_vx(i,j) / (r(i,j) + 0.5*r(i,j))
        v_xr(i  ,j) = v(i,j) - 0.5d0*(1.0d0 + dtdx*u(i,j))*ldelta_vx(i,j) / (r(i,j) - 0.5*r(i,j))
  ...
  do j = jlo-1, jhi+1
     do i = ilo-1, ihi+1
        ...
        uv_x = ubar*(v_xint(i+1,j) - v_xint(i,j)) / r(i,j)
        ...
        uu_x = ubar*(u_xint(i+1,j) - u_xint(i,j)) / r(i,j)
